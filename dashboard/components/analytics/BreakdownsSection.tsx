...
function Group({ label, intro, children }: { label: string; intro: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: "20px" }}>
      <h3 style={{ fontSize: "12px", fontWeight: 800, color: "#6b7480", letterSpacing: ".4px", margin: "4px 0 2px" }}>
        {label}
      </h3>
      <div style={{ fontSize: "13px", color: "#6b7480", margin: "0 0 12px" }}>{intro}</div>
  ...
  return (
    <div>
      ...
      <Group label="REVIEWS" intro="How the reviewer scored open jobs against your profile.">
        <SimpleBarCard title="Fit-score distribution" data={d.fitScore} color="#22a06b" allTicks />
        <HBarCard title="Approvals by industry" data={hz(d.approvalsByIndustry)} color="#22a06b" />
        <HBarCard title="Approvals by role category" data={hz(d.approvalsByRole)} color="#22a06b" />
      ...
      <Group label="COMPANIES" intro="Who is being tracked and why some were flagged.">
        ...
        <HBarCard title="Top tech tags" data={mergeTags(d.topTechTags)} color="#7a8699" />
        <HBarCard title="Top red flags" subtitle="Reasons companies were flagged during classification." data={redFlagBars} color="#e0607e" />
