...
describe("board workspace design contract", () => {
  ...
  test("filters expose a compact scroll-safe toolbar and use the shared segmented control", () => {
    ...
    // Category, Match, Location, Source, Industry, Size, Country, Sort — eight radio/checkbox
    // menus. Pay is now a slider dialog (PayRangeSlider), so it no longer contributes a
    // filter-option button.
    expect(source.match(/className="rf-board-filter-option rf-focusable"/g)).toHaveLength(8);
  });
