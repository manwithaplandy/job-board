...
4. Validates four environment secrets without printing them:
   - `VISUAL_AUTH_EMAIL`
   - `VISUAL_AUTH_PASSWORD`
   - `VISUAL_ONBOARDING_EMAIL`
   - `VISUAL_ONBOARDING_PASSWORD`
5. Runs a Playwright setup project against `github.event.deployment_status.environment_url`.
6. Signs in through the real `/login` form twice using isolated browser contexts.
7. Verifies the established account lands on the board and can render profile routes.
8. Verifies the profile-less account lands on `/onboarding`.
9. Writes both storage states under ignored, ephemeral `test-results/visual-auth/` paths.
10. Runs the full authenticated route/theme/viewport matrix using those files.
11. Uploads traces, diffs, and missing-snapshot actual images on failure, but never uploads storage-state files.
12. Deletes the temporary states in an `always()` cleanup step.
...
- The workflow checks the deployment creator/environment and exact URL before entering the protected job.
- Credentials are scoped only to the authentication setup step.
