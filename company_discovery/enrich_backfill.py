...
Scope: companies with display_name IS NULL that are still worth screening — either
active, or whose effective verdict is 'unknown' (an un-classified company that
grounding could rescue; a company with no review at all is effectively 'unknown').
Already-enriched rows (display_name set) are excluded, so the run is
resumable/idempotent and a later pass retries any board that was transiently dead.
...
# instead of 'exclude'). display_name IS NULL makes it resumable/idempotent.
_SCOPE_SQL = """
...
def main() -> None:
    ...
    try:
        ...
        log.info("enrichment scope: %s companies (display_name IS NULL, active-or-unknown)",
                 len(rows))
        updated = 0
