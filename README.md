...
| `REVIEW_DEFAULT_MODEL_PRO` | `openai/gpt-5.6-luna` | Default stage-2 model for **Pro** users who haven't picked one. Set identically on the reviewer and dashboard services so the enforced and displayed daily caps agree. The resolved model is still tier-gated and metered, so an unassigned model (like this one) meters at the Pro premium cap. |
