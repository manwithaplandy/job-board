...
describe("shared settings primitives", () => {
  ...
  test("uses stable classes backed by 44px minimum targets at every viewport", () => {
    ...
    expect(css).toMatch(/\.settings-nav-link[^}]*min-height:\s*var\(--target-size\)/s);
    expect(globalCss).toMatch(/--target-size:\s*44px/);
    expect(css).toMatch(/\.profile-detail\s*\{[^}]*font-size:\s*var\(--font-size-body\)/s);
    expect(globalCss).toMatch(/--font-size-heading:\s*32px/);
    expect(globalCss).toMatch(/--font-size-body:\s*16px/);
    expect(globalCss).toMatch(/--font-size-help:\s*14px/);
    expect(css).toMatch(/@media \(max-width: 720px\)[\s\S]*\.settings-card-grid, \.field-grid\s*\{[^}]*grid-template-columns:\s*1fr/s);
