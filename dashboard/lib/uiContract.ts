...
function includesClass(node: ts.JsxOpeningLikeElement, sourceFile: ts.SourceFile, classes: string[]): boolean {
  const value = attrText(node, "className", sourceFile);
  return classes.some((className) => new RegExp(`(?:^|[^A-Za-z0-9_-])${className.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?:$|[^A-Za-z0-9_-])`).test(value));
}
function isNumericGeometry(style: ts.JsxAttribute, sourceFile: ts.SourceFile): boolean {
  ...
  return expression.properties.some((property) => {
    ...
    return ts.isNumericLiteral(value) || (ts.isStringLiteral(value) && (/-?\d+(?:\.\d+)?(?:px|rem|em|%)?$/.test(value.text) || /\b(?:calc|min|max|clamp)\s*\(/.test(value.text)));
  });
}
...
function auditTsx(file: string, source: string, fixture: boolean): UiContractViolation[] {
  ...
  const visit = (node: ts.Node) => {
    if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
      ...
      const composite = Boolean(attr(node, "data-ui-contract-composite")) || withinMarker(node, "data-ui-contract-composite") || includesClass(node, sourceFile, COMPOSITE_CLASSES) || (fieldChild && ["input", "select", "textarea"].includes(tag));
      ...
      if (style && isNumericGeometry(style, sourceFile) && !GEOMETRY_SCOPES.has(`${file}:${renderScope(node, sourceFile)}`) && !attr(node, "data-ui-contract-geometry") && !withinMarker(node, "data-ui-contract-geometry-scope")) result.push(violation("inline-geometry", file, source, style.getStart(sourceFile), "Move geometry to shared CSS or mark the exact node, or the smallest data-driven composite scope, with a reason."));
...
function auditCss(file: string, source: string): UiContractViolation[] {
  ...
  for (const block of clean.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    ...
    const interactive = /(^|[\s,>+~])(?:a|button|input|select|textarea)(?:[\s.#[:>+~]|$)|\[role\s*=\s*["']?(?:button|menuitem|tab|radio|option)/i.test(selector) || /:(?:hover|focus|active)/.test(selector) || /(?:^|[.#_-])(?:button|btn|cta|action|link|trigger|control)(?:$|[.#:_-])/i.test(selector);
